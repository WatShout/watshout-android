...
public class CalendarAdapter  extends RecyclerView.Adapter<CalendarAdapter.ViewHolder> {
    ...
    CalendarAdapter(List<CalendarItem> listItems, Context context) {
    ...
    public void onBindViewHolder(@NonNull final CalendarAdapter.ViewHolder holder, int position) {
        ...
        CalendarItem calendarItem = listItems.get(position);

        Long time = Long.parseLong(calendarItem.getTime());

        String formattedTime = epochToIso8601(time);

        holder.mTime.setText(formattedTime);

        loadMapImage(calendarItem.getImageURL(), holder.mMap);
